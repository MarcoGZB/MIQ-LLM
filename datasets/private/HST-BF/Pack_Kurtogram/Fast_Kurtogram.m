function [c,f,S] = Fast_Kurtogram(x,nlevel,Fs,opt1,opt2)
...
if opt1 == 1  % 分叉树滤波结果
   ...
   [I,J,M] = max_IJ(Kwav); % 求最大值
%    imagesc(freq_w,1:2*nlevel,Kwav),colorbar; %绘图
...
%    imagesc(freq_stft,1:2*nlevel,Kstft),colorbar;
   [I,J,M] = max_IJ(Kstft);	
   fi = (J-1)/Nfft(end);	
...
%       lev = input(['	Enter the optimal level (btw 0 and ',num2str(nlevel),') where to filter the signal: ']);
      if opt2 == 1
         [c,Bw,fc,i,f,S] = Find_wav_kurt(x,h,g,h1,h2,h3,nlevel,lev,fi,'kurt2',Fs);
      else
         [c,Bw,fc,i,f,S] = Find_wav_kurt(x,h,g,h1,h2,h3,nlevel,lev,fi,'kurt1',Fs);
      end
   else
