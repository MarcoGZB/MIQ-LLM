clear;clc;close all
%%
datafilefolder='D:\1.数据集\1.中心数据\ZXJB\fault_data\without_box\';
% =============== 样本选择 =============
type = "roller_crack"; % outer_crack, outer_pitting, roller_crack, cage_crack  
load = 40;             % 0,20,40
num_id = 2;           % 0,1,2,3
...
switch type
    case "outer_crack"  %% 外圈裂纹
        ...
        for k = 1:length(rpm)        % 根据转速读取数据
            ...
            for i = 1:length(outer_crack_fault)      % 根据故障程度读取数据
                ...
                datafilename = sprintf('%s_%smm_%s_without_box_%d_%d_%d.csv', type, fault_id, position_id, rpm_id, load, num_id);
    ...
    case "outer_pitting" %% 外圈点蚀
        ...
        for k = 1:length(rpm)        % 根据转速读取数据
            ...
            for i = 1:length(outer_pit_fault)      % 根据故障程度读取数据
                ...
                datafilename=sprintf('%s_%s_without_box_%d_%d_%d.csv', type, outer_pit_id, rpm_id, load, num_id); % 外圈点蚀
                datafile = fullfile(datafilefolder, datafilename);
                raw_data = importdata(datafile);
    ...
    case "roller_crack"   %% 滚动体裂纹
        ...
        for k = 1:length(rpm)        % 根据转速读取数据
            ...
            for i = 1:length(roller_cr_fault)      % 根据故障程度读取数据
                ...
                datafilename=sprintf('%s_%smm_without_box_%d_%d_%d.csv', type, roller_cr_id, rpm_id, load, num_id); % 滚动体裂纹
                datafile = fullfile(datafilefolder, datafilename);
                raw_data = importdata(datafile);
    ...
    case "cage_crack"   %% 保持架裂纹
        ...
        for k = 1:length(rpm)        % 根据转速读取数据
            rpm_id  = rpm(k);
            datafilename=sprintf('%s_withoutbox_%d_%d_%d.csv', type, rpm_id, load, num_id); % 滚动体裂纹
            datafile = fullfile(datafilefolder, datafilename);
            raw_data = importdata(datafile);
